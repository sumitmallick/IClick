...
from .forms import AddVoter, EC_login_Form, AddEmployee1, AddEmployee2, AddCandidate, Vote_login_form, Vote_form,\
    Result_form
...
def CandidatesView(request):
    cand_list = Candidate.objects.raw('select Candidate_id,Candidate_name,Party_id_id,Address,Candidate_Email,'
                                      'Candidate_phn_no,No_of_votes from ovms_candidate')
...
def ResultsView(request):
    ...
    if res_form.is_valid():
        ...
        for r in all_list:
            if r.ConstituencyID_id == C_ID:
                cand_list = Candidate.objects.raw('select Candidate_id,Candidate_name,Party_id_id,No_of_votes'
                                                  ' from ovms_candidate where ConstituencyID_id = %s order by'
                                                  ' No_of_votes DESC', [C_ID])
                res_form = Result_form(request.POST or None)
                context2 = {
                    "cand_list": cand_list,
                    "res_form": res_form
